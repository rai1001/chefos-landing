...
export default function Home() {
  return (
    <main className="relative min-h-screen w-full overflow-x-hidden text-white flex flex-col font-sans selection:bg-resto-peach selection:text-white">
      
      <div className="relative w-full min-h-screen flex flex-col">
        {/* Navbar Overlay */}
        <nav className="relative z-10 flex items-center justify-between px-8 py-6 max-w-7xl mx-auto w-full">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-resto-peach/80 border border-white/10" />
            <span className="text-2xl font-serif tracking-tight text-white">RestoOs</span>
          </div>
        ...
        <div className="relative z-10 flex-1 flex flex-col items-center justify-start pt-20 px-4 pb-24">
          ...
          {/* Badge */}
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-resto-peach/10 border border-resto-peach/20 text-resto-peach text-sm font-semibold mb-8 shadow-sm backdrop-blur-md">
            <span className="flex h-2 w-2 rounded-full bg-resto-peach animate-pulse"></span>
            Inteligencia Artificial para HORECA
          </div>

          {/* Headlines */}
          <h1 className="text-5xl md:text-7xl font-serif font-medium text-center tracking-tight max-w-4xl leading-[1.1] mb-6 text-white drop-shadow-2xl">
            Automatiza tu servicio sin perder <br className="hidden md:block"/>
            <span className="text-resto-peach font-serif italic">el trato humano</span>
          </h1>
          ...
          <div className="w-full max-w-7xl grid grid-cols-1 md:grid-cols-2 gap-8 z-20">
            <VideoDemoCard 
              title="Demo de Agente Administrativo" 
              description="Aprende cómo el Agente gestiona tus facturas y escandallos automáticamente desde la sombra."
            />
            <VideoDemoCard 
              title="Demo de AutoChef (WhatsApp)" 
              description="Descubre cómo el módulo externo de comunicación atiende WhatsApp y gestiona el libro de reservas digital de forma autónoma."
            />
          </div>
        </div>
      </div>
